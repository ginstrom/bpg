...
- `bpg suggest-fix <process_file> [--json] [--providers-file <path>]`
- `bpg test <suite-file> [--json]`
...
- `fmt`
  ...
  - `--no-write` prints formatted YAML to stdout.
- `apply-patch`
  - Applies JSON patch operations to process YAML using `$`-prefixed paths.
  - Writes canonicalized YAML back to file by default.
- `suggest-fix`
  - Uses diagnostics to emit suggested JSON patch repairs.
  - `--json` returns `{ok, errors, suggestions}`.
- `test`
  - Runs spec-level tests with fixture inputs and mocked node outputs.
  - `--json` emits per-case pass/fail plus summary counts.
- `show`
